#!/usr/bin/env python3
"""Validate using FLOP-level zones (84 zones) vs NESO boundary flows."""
...
# Build FLOP-level network
flop_zones = sorted(set(v for v in sub_to_flop.values() if v != 'UNKNOWN'))
n_fz = len(flop_zones)
fz_to_idx = {z: i for i, z in enumerate(flop_zones)}

B = lil_matrix((n_fz, n_fz))
branch_data = []

for br in branches:
    fz1 = sub_to_flop.get(br['sub1'])
    fz2 = sub_to_flop.get(br['sub2'])
    if not fz1 or not fz2 or fz1 == 'UNKNOWN' or fz2 == 'UNKNOWN': continue
    if fz1 == fz2: continue
    fi, fj = fz_to_idx.get(fz1), fz_to_idx.get(fz2)
    ...
    B[fi, fj] -= b; B[fj, fi] -= b
    B[fi, fi] += b; B[fj, fj] += b
    branch_data.append((fi, fj, b, br['sub1'], br['sub2'], fz1, fz2))
...
act_branches = []
for fi, fj, b, s1, s2, fz1, fz2 in branch_data:
    ...
    act_branches.append((ai, aj, b, s1, s2, fz1, fz2))

# Slack: GZ18 area
slack_candidates = []
for code, gz in sub_to_flop.items():
...
print(f'FLOP model: {m} active zones, {len(act_branches)} branches')
...
for bname, bdata in boundary_mapping.get('boundary_links', {}).items():
    ...
    crossings = []
    for bi, (ai, aj, b, s1, s2, fz1, fz2) in enumerate(act_branches):
        if fz1 in north_gz and fz2 in south_gz: crossings.append((bi, 1))
        elif fz1 in south_gz and fz2 in north_gz: crossings.append((bi, -1))
